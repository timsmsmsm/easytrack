...
5.  If the results are not satisfactory, you can try to improve the segmentation with our ``Clean Segmentation`` and ``Remove Small Objects`` buttons.
    5.1. The ``Clean Segmentation`` button keeps the largest connected component per label on each frame, and reassigns smaller fragments and small labels to the closest neighbour identifier (cell).
    5.2. The ``Remove Small Objects`` button removes objects smaller than a specified size threshold.
6.  If none of the presets work well for your data, you can customise the tracking parameters by using our ``Parameter Tuning`` widget.
7.  The ``Parameter Tuning`` widget requires a ground truth dataset to compare the tracking results against.
    7.1. You can load your own ground truth dataset by selecting it on the ``Ground Truth Layer`` dropdown menu.
    7.2. Select a ``Study Name``. It is autogenerated, but you can customise it. You can continue a previous run/study by making the study name the same as the previous study name. This is how you can run a study that starts with, for example, 64 random trials and then another 64 trials using the TPE (Tree-Structured Parzen Estimator). We have found this 64 + 64 or 32 + 64 combination to work well.
    ...
    7.4. Select the ``Timeout`` (in seconds, default is 60s). For larger datasets or older computers, you might want to increase this value. 
        7.4.1. If the timeout is reached during a trial, a penalty score will be assigned to that trial. The penalty score is defined by the ``Timeout Penalty`` parameter (default is 10,000). The timeout penalty should be set a little higher than the worst possible value (AOGM) for your specific dataset.
    7.5. ``Sampler`` is inherited from the ``optuna`` library. Default is ``tpe``. The other option is ``random``.
    7.6. You can tick on ``Parallel`` to enable parallel processing (if your computer has multiple cores).
    7.7. ``Output Dir`` is the directory where the temporal tuning results will be saved. No need to change this unless you want to save the results in a specific location.
